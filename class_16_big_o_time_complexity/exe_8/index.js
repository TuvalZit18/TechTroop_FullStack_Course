...
const findIndex = function (numbers, num) {
  ...
  while (low <= high) {
    ...
    if (midValue < num) low = mid + 1; //right half
    if (midValue > num) high = mid - 1; // left half
    if (midValue === num) return mid;
  }
  return -1;
};
const findIndexRec = function (low, high, numbers, target) {
  if (low > high) return -1;
  let mid = low + Math.floor((high - low) / 2);
  let midValue = numbers[mid];
  if (midValue === target) return mid;
  if (midValue < target) return findIndexRec(mid + 1, high, numbers, target); //right half
  if (midValue > target) return findIndexRec(low, mid - 1, numbers, target); //left half
};

console.log(findIndexRec(0, numbers.length - 1, numbers, 2630));
